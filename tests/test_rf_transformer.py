...
class RFHybridTransformerTests(unittest.TestCase):
    ...
    def test_pure_transformer_registered(self):
        self.assertIs(MODEL_REGISTRY["transformer"], RFTransformerAutoencoder)

    def test_pure_transformer_single_channel_shape(self):
        model = build_rf_autoencoder("transformer", input_len=256, latent_dim=64, in_channels=1)
        model.eval()
        x = torch.randn(2, 1, 256)
        ...
        self.assertEqual(tuple(recon.shape), (2, 1, 256))
        self.assertEqual(tuple(latent.shape), (2, 64))
        self.assertEqual(tuple(tokens.shape), (2, 16, 96))
        self.assertEqual(model.num_time_tokens, 16)
        self.assertEqual(model.total_tokens, 16)

    def test_pure_transformer_three_channel_shape_and_loss(self):
        model = build_rf_autoencoder("transformer", input_len=256, latent_dim=64, in_channels=3)
        model.eval()
        x = torch.randn(2, 3, 256)
        ...
        self.assertEqual(tuple(recon.shape), (2, 3, 256))
        self.assertEqual(tuple(latent.shape), (2, 64))
        self.assertEqual(tuple(tokens.shape), (2, 48, 96))
        self.assertEqual(model.total_tokens, 48)
        self.assertTrue(torch.isfinite(loss))
        self.assertGreaterEqual(comps["coherence"], 0.0)
